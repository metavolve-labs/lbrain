...
# --------------------------------------------------------------------------- #
# Codex profile integration                                                    #
# --------------------------------------------------------------------------- #

_CODEX_PROFILE_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$")


def write_codex_profile(codex_home: Path, profile: str, brain_home: Path,
                        persona: str | None = None) -> tuple[Path, bool]:
    """Create a named Codex profile that injects this LBrain home into every
    tool subprocess.

    A shell bootstrap cannot export into an already-running Codex process, and
    a global setting can cross-wire multiple personas. Named profiles are the
    narrow seam: the user opts in with ``codex --profile <name>`` and Codex's
    supported ``shell_environment_policy.set`` carries the selection into each
    spawned command.

    The operation is additive and idempotent. An existing, different profile
    is never merged or overwritten because doing so could silently discard
    unrelated Codex policy.
    """
    if not _CODEX_PROFILE_RE.fullmatch(profile):
        raise ValueError(
            "profile must be 1-64 characters: letters, digits, underscore, hyphen"
        )

    codex_home = codex_home.expanduser().resolve()
    brain_home = brain_home.expanduser().resolve()
    if not (brain_home / "config.toml").is_file():
        raise ValueError(
            f"{brain_home} is not a provisioned LBrain home (config.toml missing)"
        )

    env = {"LBRAIN_HOME": str(brain_home)}
    if persona:
        env["LBRAIN_PERSONA"] = persona
    body = (
        "# Generated by `lbrain setup codex-profile`.\n"
        "# Select explicitly: codex --profile " + profile + "\n"
        "# The selected brain must be writable inside the Codex sandbox because SQLite uses WAL.\n\n"
    ...
    if path.exists():
        ...
        raise FileExistsError(
            f"refusing to overwrite existing Codex profile: {path}"
        )
