...
**Owner:** Claude (CTO) under Tad's standing directive (2026-07-24 handoff: "LBrain upgrades = the enabler").
...
- **Config.write is lossy today** (verified: drops `abstraction_topk_cap`/`abstraction_recency_guard`
  it loads, plus unknown keys + comments incl. Tad's authorization note). This change: (a) adds the
